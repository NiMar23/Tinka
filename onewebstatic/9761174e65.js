(function($) {
    ...
    function fixVimeoWidgetsHeight() {
        var vimeoWidgets = $('iframe[data-kind="WIDGETS_VIMEO"]'),
            styles = '';
        vimeoWidgets.each(function(index, elt) {
            var height = $(elt).outerWidth() * (parseFloat(elt.height) / parseFloat(elt.width));
            styles += 'div[data-id="' + getParentId(elt.parentElement) + '"] .videoFixRatio { height: ' + height + 'px;} ';
            $(elt).addClass('videoFixRatio')
        });
        return styles
    }
        ...
        tableNodes.each(function(_, node) {
            ...
            if (cols.length > 1 && firstColWidth + tableFirstCellReductionWidth > availableWidth) {
                var id = tableNode.attr('data-id'),
                    newWidth = availableWidth - tableFirstCellReductionWidth,
    ...
    var updateTextView = function(componentEle, textSize) {
        ...
        nodes.forEach(function(domNode) {
            var node = $(domNode),
                ...
                removeCls = domNode.classList.toString().split(' ').filter(function(cls) {
            ...
            node.css({
                'font-size': (fontSize > 9 ? fontSize : 9) + 'px'
            })
        })
    };
    ...
    function fixBackgroundHeights() {
        var emptyBoxWithAssetCmps = $('[data-specific-kind="BACKGROUND"] .emptyBoxWithAsset');
        emptyBoxWithAssetCmps.each(function(index, elt) {
            if ($(elt).outerWidth() !== $(elt).data('width')) {
                var height = $(elt).outerWidth() * ($(elt).data('height') / $(elt).data('width'));
                $(elt).css('height', height);
                $(elt).css('min-height', height)
            }
        })
    }

    function setHoverboxBackgroundInMobileView() {
        $('[data-specific-kind="HOVERBOX"] .hasChildren').each((i, elt) => {
            let dataWidth = parseInt(elt.getAttribute('data-width')),
                dataHeight = parseInt(elt.getAttribute('data-height')),
                rect = elt.getBoundingClientRect();
            if (rect.height === 0 && dataWidth && dataHeight) {
                $(elt).removeClass('hasChildren');
                elt.style.minHeight = rect.width * dataHeight / dataWidth + 'px'
            }
        })
    }
    var templateElt = $('.template'),
        ...
        isDesktopView = $().isDesktopPreview() || !isMobileView || !isMobileWidth;
    ...
    function run() {
        ...
        function move() {
            ...
            var blocks = $("div[data-id][data-kind$='Block']"),
                components = $("div[data-id][data-kind$='Component']"),
                componentEltsMap = {},
                blockEltsMap = {},
                groupsEltsMap = {},
                groupsItemsEltsMap = {},
                i, cmpId, col, extractElts = function(elts, extractTo, requiredIds, getAll) {
            ...
            Object.keys(groups).forEach(function(groupId) {
                var groupIdParts = groupId.split('-'),
                    grpClass = 'mobileGroup ' + groupIdParts[0] + '-' + groupIdParts[1],
                ...
                groups[groupId].forEach(function(itemId) {
                    ...
                    if (scale) {
                        ...
                        var img = groupItem.find('img'),
                            ...
                            roundedScaledWrapperWidth = Math.round(scaledWrapperWidth),
                            roundedScaledWrapperHeight = Math.round(scaledWrapperHeight),
                            scaledImgWidth = width * scale / 100,
                            scaledImgHeight = height * scale / 100,
                            roundedScaledImgWidth = Math.round(scaledImgWidth),
                            roundedScaledImgHeight = Math.round(scaledImgHeight),
                            roundOffError = 0.0001;
            ...
            setHoverboxBackgroundInMobileView()
        }
        try {
            move()
        } finally {
